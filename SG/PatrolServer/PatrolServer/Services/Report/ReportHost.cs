...
namespace PatrolServer.Services.Report
{
    ...
    public class ReportHost
    {
        ...
        private static bool InitByConfig()
        {
            ...
            try
            {
                ServiceHost serviceHost = new ServiceHost(typeof(ReportService));
        ...
        private static bool InitByUser()
        {
            ...
            try
            {
                Uri baseAddress = new Uri("http://localhost:8732/PatrolService/Report/");
                ServiceHost serviceHost = new ServiceHost(typeof(ReportService));
                WebHttpBinding binding = new WebHttpBinding();
                binding.MaxBufferSize = int.MaxValue;
                binding.MaxReceivedMessageSize = int.MaxValue;
                binding.TransferMode = TransferMode.Buffered;
                ServiceEndpoint endpoint = serviceHost.AddServiceEndpoint(typeof(IReportService), binding, baseAddress);
        ...
        {
            Console.WriteLine("ReportService Start Success!");
        }
    }
}
