...
namespace PatrolServer.Services.User.Response
{
    ...
    public class ResWebLogin
    {
        ...
        public ResWebLogin()
        {
            this.function_id = ((int)MessageHelper.WebFunctionIDs.WebLogin).ToString();//注意枚举ToString直接得到字符串
            this.SetFailed();
        }
        ...
        public static UserInfo getUserInfo(DataTable source)
        {
            ...
            if (source != null && source.Rows.Count > 0)
            {
                ...
                obj.user_pwd = item[UserEntity.PropertyFlag.UserPassword.ToString()].ToString();
                obj.token = item[UserEntity.PropertyFlag.Token.ToString()].ToString();
